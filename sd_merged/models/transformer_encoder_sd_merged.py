...
from sd_merged.modules import SdMergedTransformerEncoderLayer
from fairseq.modules import transformer_layer


class SdMergedTransformerEncoder(TransformerEncoderBase):

    def __init__(self, cfg, dictionary, embed_tokens):
        super().__init__(cfg, dictionary, embed_tokens)
        self.layers = nn.ModuleList([])
        self.layers.extend(
            [self.build_encoder_layer(cfg, idx=i) for i in range(5)]
        )

    def build_encoder_layer(self, cfg, idx=0):
        if idx == 2:
            layer = SdMergedTransformerEncoderLayer(cfg)
        else:
