...
from sd.modules.transformer_layer_sd import TransformerEncoderSd


class SdTransformerEncoder(TransformerEncoderBase):

    def __init__(self, cfg, dictionary, embed_tokens):
        super().__init__(cfg, dictionary, embed_tokens)
        self.layers = nn.ModuleList([])
        self.layers.extend(
            [self.build_encoder_layer(cfg, idx=i) for i in range(6)]
        )

    def build_encoder_layer(self, cfg, idx=0):
        if idx == 3:
            layer = TransformerEncoderSd(cfg)
        else:
